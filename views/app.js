...
app.get('/admin', checkAuthenticated, checkAdmin, (req, res) => {
    // Total travelers, total admins, and the number of distinct
    // destinations used across everyone's trips.
    const countsSql = `
        SELECT
            (SELECT COUNT(*) FROM users WHERE role = 'traveler')             AS totalUsers,
            (SELECT COUNT(*) FROM users WHERE role = 'admin')                AS totalAdmins,
            (SELECT COUNT(DISTINCT destination) FROM trips WHERE destination IS NOT NULL AND destination <> '') AS totalDestinations
    `;
